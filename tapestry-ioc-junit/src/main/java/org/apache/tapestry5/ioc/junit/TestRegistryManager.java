...
/**
 * Helper class used by the {@link TapestryIOCJUnitExtension} to manage the test registry
 */
public class TestRegistryManager
{
    ...
    public TestRegistryManager(Class<?> type) throws RuntimeException
    {
        super();

        Registry annotation = type.getAnnotation(Registry.class);
        if (annotation == null)
        { throw new RuntimeException(type.getName() + " does not specify a @Registry"); }

        this.annotation = annotation;
        this.moduleDefFactories = findModuleDefFactories(type);
    }

    protected List<Method> findModuleDefFactories(Class<?> type) throws RuntimeException
    {
    ...
    protected void validateModuleDefMethod(Method method) throws RuntimeException
    {
        int modifiers = method.getModifiers();
        if (method.getParameterTypes().length != 0 || !Modifier.isStatic(modifiers)
                || !Modifier.isPublic(modifiers))
        {

            throw new RuntimeException(String.format(
                    "@ModuleDef method %s must be public static and accept no arguments",
                    method.getName()));
        }
    ...
    public org.apache.tapestry5.ioc.Registry getOrCreateRegistry() throws Exception
    {
        if (registry == null)
        {
            ...
            for (Method moduleDefFactory : moduleDefFactories)
            {
                try
                {
                    org.apache.tapestry5.ioc.def.ModuleDef moduleDef = (org.apache.tapestry5.ioc.def.ModuleDef) moduleDefFactory
                            .invoke(null);
